...
if args.dev:
    ...
    GUILD_INFO_PATH = Path("guilds_info_dev.json")
    DB_PATH = Path("data/dev.db")
    LOG_FILE_PATH = Path("dev.log")
else:
    print("Using prod environment")
    TOKEN = os.getenv("DISCORD_TOKEN")
    GUILD_INFO_PATH = Path("guilds_info.json")
    DB_PATH = Path("data/partnerup.db")
    LOG_FILE_PATH = Path("partnerup.log")
...
schedule = ScheduleTable(DB_PATH)
groups = GroupsTable(DB_PATH)
...
async def _subscribe(interaction: discord.Interaction, timeblock: Timeblock):
    try:
        schedule.insert_timeblock(timeblock, interaction.guild_id, interaction.user.id)
        timeblocks = schedule.select_timeblocks(interaction.guild_id, interaction.user.id)
        logger.info(
            f"G:{interaction.guild_id} U:{interaction.user.id} subscribed T:{timeblock.name}."
        )
        msg = (
            f"Your new schedule is `{Timeblock.display_schedule(timeblocks)}`. "
...
async def _unsubscribe(interaction: discord.Interaction, timeblock: Timeblock):
    try:
        schedule.delete_timeblock(timeblock, interaction.guild_id, interaction.user.id)
        timeblocks = schedule.select_timeblocks(interaction.guild_id, interaction.user.id)
        logger.info(
            f"G:{interaction.guild_id} U:{interaction.user.id} unsubscribed T:{timeblock.name}."
        )
        msg = f"Your new schedule is `{Timeblock.display_schedule(timeblocks)}`."
        await interaction.response.send_message(msg, ephemeral=True)
    except Exception as e:
        logger.error(e, exc_info=True)
        await interaction.response.send_message("Unexpected Error", ephemeral=True)
...
async def _unsubscribe_all(interaction: discord.Interaction):
    try:
        schedule.delete_user(interaction.guild_id, interaction.user.id)
        logger.info(
...
async def _set_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    try:
        with open(GUILD_INFO_PATH, "r+") as f:
...
async def _pairwith(interaction: discord.Interaction, user: discord.Member):
    try:
        guild_to_channel = json.load(open(GUILD_INFO_PATH, "r"))
...
async def create_group_thread(
    ...
    thread_id = groups.select_thread_id(guild_id, userids)
    thread = None
    ...
    if thread_id is not None:
        ...
        try:
            ...
            groups.delete_group(guild_id, userids)
    ...
    if thread is None:
        ...
        groups.insert_group(guild_id, userids, channel.id, thread.id)
    else:
        logger.debug(f"Found existing thread {thread_id} for G:{guild_id} U:{userids}")
        guild = discord_client.get_guild(guild_id)
        thread = await guild.fetch_channel(thread_id)
...
async def pair(guild_id: int, timeblock: Timeblock):
    try:
        userids = schedule.select_userid(guild_id, timeblock)
        users = [discord_client.get_user(userid) for userid in userids]
        ...
        guild_to_channel = json.load(open(GUILD_INFO_PATH, "r"))
        channel = discord_client.get_channel(guild_to_channel[str(guild_id)])
        if len(users) < 2:
            for user in users:
                logger.info(
...
def local_setup():
    try:
        ...
        with open(GUILD_INFO_PATH, "w") as f:
            json.dump({}, f)
